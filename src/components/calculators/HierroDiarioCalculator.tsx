...
import { calcularHierroDiario } from '../../lib/calculators';
import ShareButtons from '../ui/ShareButtons';

export default function HierroDiarioCalculator() {
  const [edad, setEdad]   = useState('30');
  const [sexo, setSexo]   = useState<'hombre' | 'mujer'>('mujer');
  const [etapa, setEtapa] = useState<'ninguna' | 'embarazo' | 'lactancia'>('ninguna');
  const [res, setRes]     = useState<ReturnType<typeof calcularHierroDiario> | null>(null);
  ...
  function calcular() {
    ...
    setError('');
    setRes(calcularHierroDiario(e, sexo, sexo === 'mujer' ? etapa : 'ninguna'));
  }
  ...
  return (
    <div style={{ padding: 'clamp(16px, 4vw, 32px)', maxWidth: '640px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <Button onClick={calcular}>Calcular hierro diario</Button>

      {res && (
        <div style={{ marginTop: '8px' }}>
          <div style={{ background: 'var(--ink)', padding: '24px' }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: '#999', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '4px' }}>Hierro recomendado</div>
          ...
          <ShareButtons text={`Mi hierro diario recomendado es ${res.rdaMg} mg. Calcula el tuyo en CalcFit:`} url="https://www.calcfit.com/hierro-diario" />
        </div>
      )}
    </div>
  );
