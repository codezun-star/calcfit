...
import { calcularCaloriasPesas } from '../../lib/calculators';
import ShareButtons from '../ui/ShareButtons';

export default function CaloriasPesasCalculator() {
  const [peso, setPeso]       = useState('70');
  const [minutos, setMinutos] = useState('45');
  const [intensidad, setIntensidad] = useState<'general' | 'vigoroso' | 'circuito'>('vigoroso');
  const [res, setRes]         = useState<ReturnType<typeof calcularCaloriasPesas> | null>(null);
  ...
  function calcular() {
    ...
    setError('');
    setRes(calcularCaloriasPesas(p, m, intensidad));
  }
  ...
  return (
    <div style={{ padding: 'clamp(16px, 4vw, 32px)', maxWidth: '640px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '8px' }}>Tipo de sesión</div>
        <div style={{ display: 'flex', gap: '4px' }}>
          <button onClick={() => setIntensidad('general')} style={seg(intensidad === 'general')}>Suave</button>
          <button onClick={() => setIntensidad('vigoroso')} style={seg(intensidad === 'vigoroso')}>Vigorosa</button>
          <button onClick={() => setIntensidad('circuito')} style={seg(intensidad === 'circuito')}>Circuito</button>
        </div>
      </div>
      ...
      {res && (
        <div style={{ marginTop: '8px' }}>
          ...
          <ShareButtons text={`Levantando pesas quemé ${res.calorias} kcal en ${minutos} min. Calcula las tuyas en CalcFit:`} url="https://www.calcfit.com/calorias-pesas" />
        </div>
      )}
    </div>
  );
