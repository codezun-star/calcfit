...
import { calcularCaloriasSaltarCuerda } from '../../lib/calculators';
import ShareButtons from '../ui/ShareButtons';

export default function CaloriasSaltarCuerdaCalculator() {
  const [peso, setPeso]         = useState('70');
  const [minutos, setMinutos]   = useState('15');
  const [intensidad, setIntensidad] = useState<'lento' | 'moderado' | 'rapido'>('moderado');
  const [res, setRes]           = useState<ReturnType<typeof calcularCaloriasSaltarCuerda> | null>(null);
  ...
  function calcular() {
    ...
    setError('');
    setRes(calcularCaloriasSaltarCuerda(p, m, intensidad));
  }
    ...
    <div style={{ padding: 'clamp(16px, 4vw, 32px)', maxWidth: '640px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div>
        ...
        <div style={{ display: 'flex', gap: '4px' }}>
          <button onClick={() => setIntensidad('lento')} style={seg(intensidad === 'lento')}>Suave</button>
          <button onClick={() => setIntensidad('moderado')} style={seg(intensidad === 'moderado')}>Moderada</button>
          <button onClick={() => setIntensidad('rapido')} style={seg(intensidad === 'rapido')}>Rápida</button>
        </div>
      </div>
      ...
      {res && (
        <div style={{ marginTop: '8px' }}>
          ...
          <ShareButtons text={`Saltando la cuerda quemé ${res.calorias} kcal en ${minutos} min. Calcula las tuyas en CalcFit:`} url="https://www.calcfit.com/calorias-saltar-cuerda" />
        </div>
      )}
    </div>
  );
