...
import { calcularMagnesioDiario } from '../../lib/calculators';
import ShareButtons from '../ui/ShareButtons';

export default function MagnesioDiarioCalculator() {
  const [edad, setEdad]   = useState('30');
  const [sexo, setSexo]   = useState<'hombre' | 'mujer'>('hombre');
  const [etapa, setEtapa] = useState<'ninguna' | 'embarazo' | 'lactancia'>('ninguna');
  const [res, setRes]     = useState<ReturnType<typeof calcularMagnesioDiario> | null>(null);
  ...
  function calcular() {
    ...
    setError('');
    setRes(calcularMagnesioDiario(e, sexo, sexo === 'mujer' ? etapa : 'ninguna'));
  }
  ...
  return (
    <div style={{ padding: 'clamp(16px, 4vw, 32px)', maxWidth: '640px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <Button onClick={calcular}>Calcular magnesio diario</Button>

      {res && (
        <div style={{ marginTop: '8px' }}>
          <div style={{ background: 'var(--ink)', padding: '24px' }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: '#999', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '4px' }}>Magnesio recomendado</div>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: '72px', color: 'var(--acid)', lineHeight: 1 }}>{res.rdaMg}</div>
            <div style={{ fontSize: '11px', color: '#aaa' }}>mg/día (dieta)</div>
          </div>
          ...
          <ShareButtons text={`Mi magnesio diario recomendado es ${res.rdaMg} mg. Calcula el tuyo en CalcFit:`} url="https://www.calcfit.com/magnesio-diario" />
        </div>
      )}
    </div>
  );
