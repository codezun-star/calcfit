...
import { calcularVitaminaCDiaria } from '../../lib/calculators';
import ShareButtons from '../ui/ShareButtons';

export default function VitaminaCDiariaCalculator() {
  ...
  const [res, setRes]     = useState<ReturnType<typeof calcularVitaminaCDiaria> | null>(null);
  ...
  function calcular() {
    ...
    setError('');
    setRes(calcularVitaminaCDiaria(e, sexo, sexo === 'mujer' ? etapa : 'ninguna', fumador));
  }
  ...
  return (
    <div style={{ padding: 'clamp(16px, 4vw, 32px)', maxWidth: '640px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '8px' }}>¿Fumas?</div>
        <div style={{ display: 'flex', gap: '4px', maxWidth: '260px' }}>
          <button onClick={() => setFumador(false)} style={seg(!fumador)}>No</button>
          <button onClick={() => setFumador(true)} style={seg(fumador)}>Sí (+35 mg)</button>
        </div>
      </div>
      {error && <p style={{ color: '#F87171', fontSize: '13px' }}>{error}</p>}
      <Button onClick={calcular}>Calcular vitamina C</Button>

      {res && (
        <div style={{ marginTop: '8px' }}>
          <div style={{ background: 'var(--ink)', padding: '24px' }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: '#999', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '4px' }}>Vitamina C recomendada</div>
          ...
          <ShareButtons text={`Mi vitamina C diaria recomendada es ${res.rdaMg} mg. Calcula la tuya en CalcFit:`} url="https://www.calcfit.com/vitamina-c-diaria" />
        </div>
      )}
    </div>
  );
