...
import { calcularCaloriasBailando } from '../../lib/calculators';
import ShareButtons from '../ui/ShareButtons';

export default function CaloriasBailandoCalculator() {
  const [peso, setPeso]       = useState('70');
  const [minutos, setMinutos] = useState('45');
  const [estilo, setEstilo]   = useState<'social' | 'zumba' | 'intenso'>('zumba');
  const [res, setRes]         = useState<ReturnType<typeof calcularCaloriasBailando> | null>(null);
  ...
  function calcular() {
    ...
    setError('');
    setRes(calcularCaloriasBailando(p, m, estilo));
  }
  ...
  return (
    <div style={{ padding: 'clamp(16px, 4vw, 32px)', maxWidth: '640px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '8px' }}>Estilo de baile</div>
        <div style={{ display: 'flex', gap: '4px' }}>
          <button onClick={() => setEstilo('social')} style={seg(estilo === 'social')}>Social/salsa</button>
          <button onClick={() => setEstilo('zumba')} style={seg(estilo === 'zumba')}>Zumba</button>
          <button onClick={() => setEstilo('intenso')} style={seg(estilo === 'intenso')}>Intenso</button>
        </div>
      </div>
      ...
      {res && (
        <div style={{ marginTop: '8px' }}>
          ...
          <ShareButtons text={`Bailando quemé ${res.calorias} kcal en ${minutos} min. Calcula las tuyas en CalcFit:`} url="https://www.calcfit.com/calorias-bailando" />
        </div>
      )}
    </div>
  );
